...
void cherche_homonymes(t_promo p, char* nom){
    t_etudiant e;
    int retour, cont=0; 
    printf("\n\t=============== Resultats de la recherche pour la chaine ' %s ' : ===============\n",nom);
    for(int i=0; i < p.taille; i++){
        e = p.student[i];
        retour = compare_chaine(e.nom,nom);
        if(retour == 0){
            printf("\n\t -");
            affiche_etudiant(e);
            cont++;
...
void cherche_prefixe(t_promo p, t_promo* res, char* prefixe){
    ...
    for(int i=0; i < p.taille; i++){
        e = p.student[i];
        retour = est_prefixe(prefixe,e.nom);
        if(retour == 1){
            (*res).student[i] = p.student[i];
            printf("\n\t -");
            affiche_etudiant(e);
            cont++;
        }
